    ...
    <body onload="time()" class="app sidebar-mini rtl">
                                ...
                                <table class="table table-hover table-bordered js-copytextarea" cellpadding="0" cellspacing="0" border="0"
                                    ...
                                    <c:forEach items="${user}" var="u">
                                        <tr>
                                            ...
                                            <td>${u.user_name}</td>
                                            <td>${u.user_email}</td>
                                            <td id="roleCell">${u.getRole()}</td>
                                            <td><button class="btn btn-primary btn-sm edit" type="button" title="Sửa" value="${u.user_id}" role="${u.getRole()}"><i class="fas fa-edit"></i>
                                                </button>
                                                <button class="btn btn-primary btn-sm trash" type="button" title="Xóa" value="${u.user_id}"><i class="fas fa-trash-alt"></i>
                                                </button>
                                            </td>
                                        </tr>
                                    </c:forEach>
        ...
        <script>
                                            ...
                                            $('.trash').on("click", function () {
                                                Swal.fire({
                                                    ...
                                                    if (result.isConfirmed) {
                                                        ...
                                                        deleteUser($(this).closest('button').val());
                                                        deleteRow(this);
                                                    }
                                                });
                                            });
                                            ...
                                            $('.edit').on('click', function () {
                                                Swal.fire({
                                                    title: "Thay đổi vai trò của tài khoản với ID: " + $(this).closest('button').val(),
                                                    input: "select",
                                                    inputOptions: {
                                                        Admin: "Admin",
                                                        Customer: "Customer"
                                                    },
                                                    inputValue: $(this).closest('button').attr("role"),
                                                    ...
                                                    if (result.isConfirmed) {
                                                        if (result.value !== $(this).closest('button').attr("role")) {
                                                            updateUserRole($(this).closest('button').val(), result.value);
                                                            $(this).closest('tr').find('#roleCell').text(result.value);
                                                            $(this).closest('button').attr("role", result.value);
                                                            Swal.fire({
                                                                title: "Đã thay đổi!",
                                                                text: "Người dùng với ID " + $(this).closest('button').val() + " đã được thay đổi vai trò thành " + result.value,
                                                                icon: "success"
                                                            });
                                                        } else {
                                                            Swal.fire({
                                                                title: "Không thay đổi!",
                                                                text: "Người dùng với ID " + $(this).closest('button').val() + " vẫn có vai trò " + result.value,
                                                                icon: "warning"
                                                            });
                                                        }
                                                    }
